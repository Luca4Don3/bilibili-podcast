...
def _inputs(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    fake_python.write_text(
        ...
        "  printf '#!/bin/sh\\nexit 0\\n' > \"$target/bin/pip\"\n"
        "  printf '#!/bin/sh\\nexit 0\\n' > \"$target/bin/python3\"\n"
        "  for name in bilibili-podcast bilibili-podcast-admin bilibili-podcast-config bilibili-podcast-web bilibili-podcast-publish bilibili-podcast-crontab; do\n"
    ...
    return artifact, wheel, fake_python


def _prepare_command(
    root: Path, artifact: Path, wheel: Path, python: Path, *, apply: bool,
) -> list[str]:
    command = [str(SCRIPT)]
    if apply:
    ...
    command.extend([
        ...
        "--bootstrap-wheel", str(wheel),
        "--bootstrap-wheel-sha256", _sha256(wheel),
        "--python", str(python),
    ])
    return command
...
def test_release_prepare_is_dry_run_by_default_and_activation_is_explicit(tmp_path: Path) -> None:
    artifact, wheel, python = _inputs(tmp_path)
    root = tmp_path / "install"

    dry = subprocess.run(
        _prepare_command(root, artifact, wheel, python, apply=False),
    ...
    applied = subprocess.run(
        _prepare_command(root, artifact, wheel, python, apply=True),
        capture_output=True, text=True, check=True,
    )
    ...
    activate = subprocess.run([
        ...
        "--commit", COMMIT, "--python", str(python),
    ], capture_output=True, text=True, check=True)
...
def test_release_prepare_rejects_checksum_mismatch_without_writes(tmp_path: Path) -> None:
    artifact, wheel, python = _inputs(tmp_path)
    root = tmp_path / "install"
    command = _prepare_command(root, artifact, wheel, python, apply=True)
