...
class TestUnitGeneration:
    ...
    def test_env_file_value_reads_export_syntax(self, tmp_path: Path):
        env_file = tmp_path / "bilibili-podcast-env.sh"
        env_file.write_text('export BILIBILI_PODCAST_MEDIA_BASE_URL="http://media.example"\n')
        assert sysd._env_file_value(env_file, "BILIBILI_PODCAST_MEDIA_BASE_URL") == "http://media.example"
...
class TestApplyRollback:
    ...
    def test_restart_timer_failure_rollbacks_and_restores_active_state(self, tmp_path: Path):
        ...
        import os
        os.environ["BILIBILI_PODCAST_SYSTEMD_DIR"] = str(tmp_path)
