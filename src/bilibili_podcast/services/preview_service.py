...
class PreviewService:
    ...
    def run_dry_run(
        ...
        if not sync_bin:
            return DryRunResult(
                ...
                error="未配置 BILIPOD_SYNC_PATH，无法执行干跑。",
            )
    ...
    def run_preview(self, series: str, **overrides) -> DryRunResult:
        ...
        args = []
        cookie_file = overrides.get("cookie_file") or os.environ.get("BILIPOD_COOKIE_FILE", "")
        if cookie_file:
        ...
        for key in ("media_root", "json_root", "rss_root", "lock_file",
            ...
            val = overrides.get(key) or os.environ.get(f"BILIPOD_{key.upper()}", "")
            if val:
