...
# ── Config ──────────────────────────────────────────────────────────────
DB_PATH = os.environ.get("BILIPOD_CONFIG_DB")
PASSWORD = os.environ.get("BILIPOD_WEB_PASSWORD")

if not DB_PATH:
    raise RuntimeError("BILIPOD_CONFIG_DB environment variable is required")
if not PASSWORD:
    raise RuntimeError("BILIPOD_WEB_PASSWORD environment variable is required")

_SECRET_KEY = hashlib.sha256(PASSWORD.encode()).hexdigest()
_COOKIE_NAME = "bilipod_session"
_SESSION_MAX_AGE = 86400  # 24 hours
_HTTPS = os.environ.get("BILIPOD_HTTPS", "") == "1"

# ── App ─────────────────────────────────────────────────────────────────
_HERE = Path(__file__).resolve().parent
_TEMPLATE_DIR = str(_HERE / "templates")
app = FastAPI(title="Bilipod Manager")
...
async def preview_run(request: Request, series: str,
    ...
    media_root = os.environ.get("BILIPOD_MEDIA_ROOT", "/tmp/bilipod-media")
    json_root = os.environ.get("BILIPOD_JSON_ROOT", "/tmp/bilipod-json")
    rss_root = os.environ.get("BILIPOD_RSS_ROOT", "/tmp/bilipod-rss")
    lock_file = os.environ.get("BILIPOD_LOCK_FILE", "/tmp/bilipod-preview.lock")
    media_base_url = os.environ.get("BILIPOD_MEDIA_BASE_URL", "http://localhost:8080")
    browser_data_root = os.environ.get("BILIPOD_BROWSER_USER_DATA_ROOT", "/tmp/bilipod-browser")
    log_dir = os.environ.get("BILIPOD_LOG_DIR", "/tmp/bilipod-log")
...
async def jobs_page(request: Request):
    ...
    log_dir = os.environ.get("BILIPOD_LOG_DIR", "")
    error_log = ""
    if log_dir:
...
async def manual_media_page(request: Request, series: str,
    ...
    json_dir = Path(os.environ.get("BILIPOD_JSON_ROOT", "/var/lib/bilipod/json")) / series
    media_dir = Path(os.environ.get("BILIPOD_MEDIA_ROOT", "/var/lib/bilipod/media")) / series
...
async def manual_media_attach(
    ...
    media_base_url = os.environ.get("BILIPOD_MEDIA_BASE_URL", "http://localhost:58743")
