...
| `api_backend` | string | 否 | 使用的 B 站 API 后端：`native`（默认）/ `bilibili-api` / `bilix` / `yutto`，支持逗号分隔降级链，见下方说明 |

### API 后端（api_backend）

| 后端 | 支持范围 | 说明 |
|------|----------|------|
| `native` | 空间 / 系列 / 剧集 | **默认后端**，自研直连，不依赖任何第三方 B 站库 |
| `bilibili-api` | 空间 / 系列 / 剧集 | 已弃用：官方已关停且不再随主依赖安装，仅迁移期可选 |
...
api_backend: "yutto,bilix"          # yutto 优先，bilix 兜底
api_backend: "native"               # 单后端写法同样支持（默认值）
```
